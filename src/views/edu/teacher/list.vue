<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="资历" prop="career" width="150">
      </el-table-column>
      <el-table-column
        prop="tag"
        label="标签"
      ...
      >
        <template slot-scope="scope">
          <el-tag
            ...
            disable-transitions
            >{{ scope.row.level == 1 ? "高级讲师" : "首席讲师" }}</el-tag
          >
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    getlist(page = 1) {
      this.page = page;
      teaApi.page(this.page, this.size).then(res => {
    ...
    handleClick(row){
        this.$router.push("/teacher/teaedit/"+row.id)
    },
    handleDelete(row) {
